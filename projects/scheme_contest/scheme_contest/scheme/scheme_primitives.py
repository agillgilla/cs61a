...
@primitive("pensize")
def tscheme_pensize(width=None):
    """Set the line thickness to width or return it. If resizemode
    is set to “auto” and turtleshape is a polygon, that polygon is 
    drawn with the same line thickness. If no argument is given, the
    current pensize is returned."""
    if width == None:
        return turtle.pensize(None)
    else:
        turtle.pensize(width)

@primitive("dot")
def tscheme_dot(size=None, *color):
