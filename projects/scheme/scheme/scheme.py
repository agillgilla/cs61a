...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            evaluated = lambda operand: scheme_optimized_eval(operand, env)
            args = rest.map(evaluated)
            if isinstance(procedure, PrimitiveProcedure):
                return procedure.apply(args, env)
            elif isinstance(procedure, LambdaProcedure):
                new_frame = procedure.make_call_frame(procedure.formals, args)
                result.env, result.expr = new_frame, procedure.body
