...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        if isinstance(insect, QueenAnt) and not insect.imposter: # Do nothing if trying to remove QueenAnt
            return
...
class ThrowerAnt(Ant):
    ...
    damage = 1
    food_cost = 3
    min_range = None # No limit to min range
    max_range = None # No limit to max range
    ...
    def nearest_bee(self, hive):
        ...
        while current_place.entrance != None:
            if random_or_none(current_place.bees) != None and (self.min_range == None or distance >= self.min_range) and (self.max_range == None or distance <= self.max_range):
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing each turn, but provides protection."""
...
class QueenAnt(ScubaThrower):  
    ...
    def action(self, colony):
        ...
        def double_damage(place):
            if not isinstance(place.ant, QueenAnt) and place.ant != None:
                    if place.ant not in self.doubled:
                        self.doubled.append(place.ant)
                        place.ant.damage *= 2
                    if place.ant.container and place.ant.ant != None:
                        if not isinstance(place.ant.ant, QueenAnt) and place.ant.ant not in self.doubled:
        ...
        if self.imposter:
            ...
            while current_place.exit != None:
                double_damage(current_place)
                current_place = current_place.exit
                    
            if isinstance(self.place.ant, TankAnt) and self.place.ant not in self.doubled:
                self.doubled.append(self.place.ant.doubled)
