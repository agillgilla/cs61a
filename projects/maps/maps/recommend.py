...
def find_closest(location, centroids):
    ...
    for centroid in centroids[1:]:
        if distance(location, centroid) < distance(location, closest):
            closest = centroid
    return closest"""
...
def group_by_centroid(restaurants, centroids):
    ...
    for restaurant in restaurants:
        ...
        closests.append([closest, restaurant])
    return group_by_first(closests)
    # END Question 4
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    x_avg = mean([restaurant_location(restaurant)[0] for restaurant in cluster])
    y_avg = mean([restaurant_location(restuarant)[1] for restuarant in cluster])
    return [x_avg, y_avg]
    # END Question 5
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in reviewed:
            restratings[name] = user_rating(user, name)
        else:
            restratings[name] = predictor(restaurant)
    return restratings
